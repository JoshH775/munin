...
- Channel = life domain (#cooking, #goals). `channel_settings` holds one row per channel with `model` and `effort` (plus mute/ephemeral flags). Memory lives in its own `memory` table, one row per channel/thread — `content` is a living document the model rewrites in place, not a log. The persona lives in `system.md` (read at startup in `messageHandler`), not the DB. The reserved `'global'` row carries the default model/effort and the global-tier memory. A missing settings row or null column inherits (parent channel, then the global row), so a new channel works with zero setup; memory reads tier the same way. Messages in threads resolve to the parent channel's row. `channel_settings` reads/writes (`resolveSettings`, config, toggles) live in `src/repositories/channelSettings.ts`; the `memory` table's read/write (`resolveMemory`, `updateMemory`) in `src/repositories/memory.ts`.
